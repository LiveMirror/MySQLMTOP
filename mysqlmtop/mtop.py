...
def main():
    ...
    monitor_slow_query = func.get_option('monitor_slow_query')
    ...
    if monitor=="1":
        ...
        if monitor_replication=="1":
            ...
            joblist.append(job)
            job.start()
        if monitor_slow_query=="1":
            job = Process(target = job_run, args = ('check_mysql_slow_query.py',2))
            joblist.append(job)
            job.start()
        if "1"=="1":
            job = Process(target = job_run, args = ('check_mysql_performance.py',frequency_monitor))
            joblist.append(job)
            job.start()
        if alarm=="1":
